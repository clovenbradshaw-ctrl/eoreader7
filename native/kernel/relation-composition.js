...
/**
 * Return a maximum set of edge-disjoint chain witnesses for one predicate pair.
 *
 * Multiple chains can be generated by one old p-edge fanning into several later
 * q-edges (or vice versa). Those are real chain sites, but they are not repeated
 * evidence that p->q composition recurs. Hyperlexicon nomination therefore uses
 * a bipartite maximum matching over left/right witnessed edges: each nominated
 * support must contribute a fresh p witness AND a fresh q witness.
 */
function independentChainIds(item, chainsById) {
  const byLeft = new Map();
  for (const id of item?.chainIds ?? []) {
    const chain = chainsById.get(id);
    if (!chain?.leftEdge?.id || !chain?.rightEdge?.id) continue;
    if (!byLeft.has(chain.leftEdge.id)) byLeft.set(chain.leftEdge.id, []);
    byLeft.get(chain.leftEdge.id).push({ id, right: chain.rightEdge.id });
  }

  const matchedRight = new Map();
  const visit = (left, seenRight) => {
    for (const candidate of byLeft.get(left) ?? []) {
      if (seenRight.has(candidate.right)) continue;
      seenRight.add(candidate.right);
      const prior = matchedRight.get(candidate.right);
      if (!prior || visit(prior.left, seenRight)) {
        matchedRight.set(candidate.right, { left, chainId: candidate.id });
        return true;
      }
    }
    return false;
  };
  for (const left of byLeft.keys()) visit(left, new Set());
  return [...matchedRight.values()].map((match) => match.chainId);
}
...
export function createRelationCompositionLedger(entries = []) {
  ...
  return {
    ...
    candidates: ({ minWitnesses = 2 } = {}) => freeze([...pairSupport.values()]
      .map((item) => ({ item, independent: independentChainIds(item, chainsById) }))
      .filter(({ independent }) => independent.length >= minWitnesses)
      .map(({ item, independent }) => freeze({
        left: item.left,
        right: item.right,
        standing: "candidate",
        witnesses: freeze(independent.map((id) => {
          const chain = chainsById.get(id);
          return freeze([chain.leftEdge.id, chain.rightEdge.id]);
        })),
        provenance: freeze({ giver: null, basis: "repeated independent witnessed relation adjacency through explicitly resolved shared referents" }),
        meta: freeze({ observed: true, support: independent.length, chainSites: item.chainIds.size }),
      }))),
    evaluate: (hyperlexicon = null) => evaluateChains([...chainsById.values()], hyperlexicon),
    diagnostics: () => {
      const pairs = [...pairSupport.values()].map((item) => {
        const independent = independentChainIds(item, chainsById);
        return freeze({
          left: item.left,
          right: item.right,
          chainSites: item.chainIds.size,
          independentSupport: independent.length,
          examples: freeze([...item.chainIds].slice(0, 5).map(chainExample).filter(Boolean)),
          independentExamples: freeze(independent.slice(0, 5).map(chainExample).filter(Boolean)),
        });
      }).sort((a, b) => b.independentSupport - a.independentSupport || b.chainSites - a.chainSites || String(a.left).localeCompare(String(b.left)) || String(a.right).localeCompare(String(b.right)));
      ...
      return freeze({
        ...
        repeatedPairTypes: pairs.filter((item) => item.independentSupport >= 2).length,
