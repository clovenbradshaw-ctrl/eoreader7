// eval/mine-1-unimorph-lemma.mjs — MINE-1 with vocabulary widened by
// UniMorph, PLUS lemma-aware verb matching (hypergraph.js's 2026-08-19
// amendment: createLemmatizer/morphologyIndex, widening `bound`'s own
// verb comparison from exact string equality to the same lemma). Same
// protocol as mine-1-unimorph.mjs otherwise, same fixture, same scoring
// rule, declared before this run and unchanged after.
//
// Run: node eval/mine-1-unimorph-lemma.mjs
...
async function main() {
  ...
  console.log(`MINE-1 · house verdict reader + UniMorph-widened vocabulary + lemma-aware verb matching`);
  ...
  const outPath = join(here, "results", "mine-1-unimorph-lemma-run.json");
  const { writeFileSync } = await import("node:fs");
  writeFileSync(outPath, JSON.stringify({ totals, examined, perEssay }, null, 1));
  console.log(`\nfull per-essay breakdown: eval/results/mine-1-unimorph-lemma-run.json`);
}
