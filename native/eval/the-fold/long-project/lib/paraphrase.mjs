...
// used ONLY to reword the zero-ground-truth distractor pool (lib/
// distractors.mjs), never the planted facts (lib/plants.mjs, which keep
// their own hand-written paraphrase pool, per PREREGISTRATION.md's ground-
// truth discipline). This is mechanical code, not a model: a fixed lookup
// table of hand-written alternative phrasings, applied deterministically by
// generate-corpus.mjs the same way every other seeded choice in this corpus
// is made (which session a plant lands on, which attendees appear, ...).
//
// WHY THIS EXISTS (sensitivity audit, 2026-09-22, "the dedup exploit"): with
// only 35 distinct distractor strings cycled through hundreds of fill slots,
// `sort session-*.md-bullets | uniq -c` perfectly separated the corpus into
// "frequency 1 == planted" and "frequency >1 == filler" at rung-60/rung-300,
// with the separation getting CLEANER as the rung grew — the opposite of
// this test's own scaling intent. Rewording each reused distractor breaks
// exact-string frequency as a planted/filler signal: the SAME routine event
// (id) now renders as one of several different sentences, so raw-text
// frequency no longer tracks planted-vs-distractor status. See
// verify-ground-truth.mjs's dedup-frequency check, which re-runs this exact
// attack after generation and fails the build if it still cleanly separates.
//
...
// as any of 3^k distinct strings, not just one of a fixed short list — so
// two distractors that share only one common word (both "found" something,
// say) still diverge everywhere else they differ. A distractor with zero
// matching rules is given an explicit manual `variants` override in lib/
// distractors.mjs instead (disclosed there) rather than silently repeating
// verbatim — hasAnyRule() below is what lib/distractors.mjs's own build step
// uses to find those.

const RULES = [
  [/\bBumped\b/, ["Bumped", "Nudged up", "Increased"]],
  [/\bbumped\b/, ["bumped", "nudged up", "increased"]],
  ...
  [/\bdeferred\b/, ["deferred", "tabled", "postponed"]],
  [/\bflagged\b/, ["flagged", "noted", "called out"]],
  [/\bqueued\b/, ["queued", "lined up", "put on the list"]],
  [/\bconfirmed\b/, ["confirmed", "verified", "double-checked"]],
  ...
  [/\bsmall\b/, ["small", "minor", "quick"]],
  [/\bminor\b/, ["minor", "small", "light"]],
  ...
  [/\bregression\b/, ["regression", "bug", "issue"]],
  [/\bworked out\b/, ["worked out", "figured out", "sorted out"]],
];
