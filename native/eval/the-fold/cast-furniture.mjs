...
// Consolidation 2026-09-25: main's cast.js (P251(b)) superseded this branch's
// `{ blanked }` call option with the `blankFurniture` organ — any truthy
// value makes every face read `chunk.blanked`. The two arms are therefore two
// indexes: one built without it (raw) and one with it (blanked).
const ORGANS = { splitSentences, extractSurfaces, discoverReferents, namesCorefer, diaNorm };
const indexRaw = makeReferentIndex(ORGANS);
const indexBlanked = makeReferentIndex({ ...ORGANS, blankFurniture: true });
const indexFor = (passages, { blanked = false } = {}) => (blanked ? indexBlanked : indexRaw)(passages);
