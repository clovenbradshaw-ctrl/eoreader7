...
// Amendment 1 (2026-09-22, see PREREGISTRATION.md): regex alts and regex
// forbidden patterns ran against the RAW answer, so the apostrophe rule
// above never reached them — "don't"/"doesn't" could not match a negator
// spelled "dont"/"doesnt". Regexes now see the answer with apostrophes
// removed and every other character (the clause bounds .;, they rely on)
// kept. No existing regex contains an apostrophe (checked before the change).
const unApos = (s) => String(s ?? "").replace(/[’']/g, "");
...
const SILENCE_MARKERS = [
  ...
  // Amendment 1 (2026-09-22): "dont" added to the negators (the commonest
  // contraction was missing outright), and name / determine / identify /
  // establish added to the verbs, in both fallbacks — "The logs don't say",
  // "The logs never name one" and "Not determined in the logs" are plain
  // silence admissions the list could not match.
  "\\b(?:not|never|no|nowhere|nothing|none|isnt|doesnt|dont|didnt|cant|cannot|couldnt|wasnt|arent)\\b[^.;,\\n]{0,45}\\b(?:stat(?:e|es|ed|ing)|mention(?:s|ed|ing)?|say[s]?|said|specif(?:y|ies|ied|ying)|disclos(?:e|es|ed|ing)|document(?:s|ed|ing)?|given?|find|finds|found|finding|written|writes|record(?:s|ed|ing)?|indicat(?:e|es|ed|ing)|answer(?:s|ed|ing)?|address(?:es|ed|ing)?|appear(?:s|ed|ing)?|show(?:s|n|ing)?|list(?:s|ed|ing)?|nam(?:e|es|ed|ing)|determin(?:e|es|ed|ing)|identif(?:y|ies|ied|ying)|establish(?:es|ed|ing)?)\\b",
  "\\b(?:stat(?:e|es|ed)|mention(?:s|ed)?|specif(?:y|ies|ied)|disclos(?:e|es|ed)|document(?:s|ed)?|given?|found|written|record(?:s|ed)?|indicat(?:e|es|ed)|answer(?:s|ed)?|address(?:es|ed)?|appear(?:s|ed)?|show(?:s|n|ing)?|nam(?:e|es|ed)|determin(?:e|es|ed)|identif(?:y|ies|ied)|establish(?:es|ed)?)\\b[^.;,\\n]{0,20}\\b(?:not|never|nowhere|anywhere)\\b",
  // Amendment 1, second pass (same day, same unaided answers, still before
  // any with-eoreader7 answer): "Not in the logs." and "The logs give no
  // formal job title." are plain silence admissions neither fallback could
  // reach — the record is named "logs"/"sessions"/"notes" as often as
  // "record", and the negator can follow a giving verb.
  "\\b(?:not|nothing|no|none)\\b[^.;,\\n]{0,25}\\bin (?:the |these |any |this )?(?:logs?|records?|sessions?|session logs|notes|meeting notes)\\b",
  "\\b(?:give|gives|gave|provide|provides|provided|list|lists|listed|contain|contains|contained|include|includes|included)\\s+no\\b",
];
...
function grade(q, answerRaw) {
  ...
  for (const g of requiredGroups) if (!hasAny(n, unApos(answer), g)) reasons.push(`missing required: ${g[0]}${g.length > 1 ? ` (or ${g.length - 1} alt.)` : ""}`);
  const forbiddenHit = forbidden.find((p) => {
    ...
    if (/[\\^$.|?*+()[\]{}]/.test(p)) return regexAny(unApos(answer), [p]);
