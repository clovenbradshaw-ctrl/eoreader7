// eval/mine-1-layered-clause.mjs — mine-1-layered.mjs's veto idea (L2's
// recall minus resolveSpanRole's real non-verb evidence), re-tried on top
// of mine-1-span-role-clause.mjs's granularity fix (clause-level frames
// instead of sentence-shared ones). The first veto attempt failed because
// its evidence was contaminated — every ambiguous word in a sentence
// inherited one shared verdict, so "resolved non-verb" wasn't really
// evidence about THAT word. With per-occurrence resolution now real
// (verb: 0 -> 121 resolutions once frames were clause-level), the veto is
// worth re-testing on genuine per-word evidence instead of sentence-topic
// classification wearing word-level clothes.
//
...
async function main() {
  ...
  for (const ex of data.examples) {
    ...
    const resolvedNonVerb = new Set();
    for (const b of bindings) {
      const word = byId.get(b.id);
      if (!word) continue;
      if (b.role === "verb") resolvedVerb.add(word);
      else resolvedNonVerb.add(word);
    }
    // The veto: real, per-occurrence counter-evidence (resolved non-verb at
    // least once) with no positive evidence offsetting it (never resolved
    // verb) — everything else keeps UniMorph's permissive default,
    // including words resolveSpanRole never formed an opinion about.
    const vetoed = new Set([...resolvedNonVerb].filter((w) => !resolvedVerb.has(w)));
  ...
  console.log(`MINE-1 · layered (UniMorph recall minus veto), CLAUSE-level frames`);
  ...
  writeFileSync(join(here, "results", "mine-1-layered-clause-run.json"), JSON.stringify({ totals, resolutionTotals, examined, perEssay }, null, 1));
  console.log(`\nfull per-essay breakdown: eval/results/mine-1-layered-clause-run.json`);
}
