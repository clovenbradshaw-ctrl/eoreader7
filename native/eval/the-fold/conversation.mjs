...
for (let turn = state.turn + 1; turn <= TURNS; turn++) {
  ...
  console.log(`[${turn}/${TURNS}] ${m.move.padEnd(8)} ${String(Math.round(row.ms / 1000)).padStart(4)}s ${String(row.calls).padStart(2)} calls  ${row.addressed === null ? "  " : row.addressed ? "✓ " : "✗ "}${cited ? "cited " : "      "}${absent ? "absent " : ""}${m.target ? `· ${trim(m.target, 24)} ` : ""}${phrasedBy === "mechanical" ? "(mech) " : ""}${row.meaningChase ? `≐${row.meaningChase.equated}/${row.meaningChase.unEquated} ` : ""}${trim(question, 70)}`);
  if (turn % 25 === 0) {
