...
## S110 — Sniff, form a thesis, test it, revise it: a fourth heading convention, found and closed the same way (2026-09-10)

**Generality:** specimen-scoped. The convention itself (`CHAPTER <roman>`, no period) is a real, now-shared fact about one book; the practice this entry records — sniff with the cheap senses first, state a working thesis, then actually test it against the expensive one before trusting it — is meant to generalize to any document this pipeline meets next.

User direction, verbatim, naming the discipline directly rather than leaving it implicit: *"we sniff then try and try to create a single working thesis on what it is we're experiencing, always revisable."* Run for real against the freshly-corrected `pg1661_The_Adventures_of_Tom_Sawyer.txt` (S107/S108's corpus-integrity pass had just fixed this file's own mislabeling — a genuinely unread specimen, not one this pipeline had any prior expectation about):

1. **Sniff, cheap first.** `structure-rec.mjs` and `table-rec.mjs` — neither one reads the whole book through the main pipeline, both just scan for structural signature. Result: `table-rec` found nothing (no embedded tables); `structure-rec` escalated past both known conventions, found a skeleton recurring 35 times with a strictly monotonic Roman numeral (`CHAPTER I` through `CHAPTER XXXV`, no model call needed), and appended a new, fourth convention to `heading-conventions.json`: `CHAPTER <roman>`, no trailing period — distinct from AIW's own `CHAPTER <roman>.` by exactly one character, and from Sherlock Holmes's `<roman>.` (S107) by having a marker word at all.
2. **A working thesis, stated plainly and labeled as one.** From the sniff alone: "this is a standard 35-chapter novel, no embedded tables, using a Roman-numeral heading convention close to but not identical with AIW's own — should read cleanly once the reader recognizes it."
3. **Tested against the expensive sense, not assumed.** `node eot-jsonl.mjs pg1661... 1` → `no chapter 1 inferred in this document`. The thesis was WRONG in its unstated half: identifying a convention and a reader KNOWING that convention are two different facts, and `eot-jsonl.mjs`'s own heading regex — a separate, hand-maintained pattern family, not yet wired to consult `heading-conventions.json` at all (S107's own named gap) — had no fourth branch for a bare Roman numeral.
4. **Revised, not re-asserted.** A third alternative added to the SAME regex family already carrying S102's Dorian-Gray fix and S107's Frankenstein fix, in `eot-jsonl.mjs` and its `recoverability.mjs` duplicate, tried only after the period-requiring form has already had its chance so nothing already verified can start matching differently. Regression-checked before trusting it: AIW (12/12, 100% recoverable, all four golden scores byte-identical), Dorian Gray (ch1-3, 100%), Frankenstein (ch1, 100%) — all unchanged.
5. **Re-tested, confirmed, not re-asserted either.** `pg1661` chapter 1 (title `"Tom!"`) now reads clean: 100% recoverable, 2380/2380 words, 5 entities admitted, 35 arrangements carrying a referent id.

This closes exactly one instance of S107's own named gap ("wiring the reader to consult this detector automatically... is real, scoped, not attempted") by hand, for one document — the general wiring (structure-rec's own discovery feeding the reader without a human re-deriving the same regex edit each time) remains real, scoped, and not attempted. What this entry adds is the discipline, exercised rather than described: a sniff's own thesis is a claim to be tested against the material, not a conclusion to build on, and this is the second time this exact session has caught its own thesis wrong before trusting it (S109's prior-arbitration bugs are the other).

## S111 — The gap named four times, actually closed: `eot-jsonl.mjs` now consults the shared detector directly (2026-09-10)

**Generality:** specimen-scoped. The wiring itself is meant universally (it is `eot-jsonl.mjs`'s own chapter-detection code, not a per-book branch); the five specimens re-verified below are what it was actually run against before being trusted.

User direction, verbatim: *"wire it all in."* S107, S108 ("what this does not yet touch"), S109, and S110 each named the same standing gap in slightly different words: a human had to notice a new book's heading convention and hand-edit `eot-jsonl.mjs`'s own regex before it could be read — three real instances of exactly that (S102's Dorian Gray title check, S107's Frankenstein fix, S110's Tom Sawyer fix), each caught and fixed correctly, and each requiring a human in the loop that `structure-rec.mjs` was built specifically to remove.

### The wiring

`structure-rec.mjs`'s `tier1`, `tier2`, `deriveConvention`, `buildRegex`, `loadLibrary`, and `saveLibrary` are now exported and imported directly by `eot-jsonl.mjs`. The chapter-detection block that used to carry its own growing regex alternation now: tries every convention in `heading-conventions.json` mechanically (tier 1); if none match, looks for a recurring skeleton with a monotonic numeral (tier 2) and, if consistent across every member of the group, builds a NEW convention on the spot AND appends it to the shared library — so the next book with the same shape is a tier-1 hit, in this file, without anyone touching it. Tier 3 (the model witness) is deliberately NOT called from here: it only ever confirms "this looks structural," never supplies a numeral, and a reader that assigns chapter ordinals needs the numeral tier 2 already requires — a real, disclosed case (a confirmed-structural convention with no numeral) this reader still cannot open a book from, named rather than forced.

### A real bug, found wiring it in, before anything downstream could be trusted

Importing `structure-rec.mjs` for its exported functions ran into the exact trap this codebase's own history already named for `eot-jsonl.mjs` itself (`grain-typing.mjs`'s own header) and had apparently forgotten while writing `structure-rec.mjs` fresh: its entire CLI section (`const bookPath = process.argv[2]; if (!bookPath) {...}`) had NO `import.meta.url` guard at all, unlike `table-rec.mjs`'s own correct pattern — so importing it from `eot-jsonl.mjs` ran `structure-rec.mjs`'s own "usage: node structure-rec.mjs..." exit against `eot-jsonl.mjs`'s argv, immediately. Fixed by wrapping the whole CLI section the same way `table-rec.mjs` already does it. Confirmed after the fix: the standalone CLI still works unmodified, and importing the functions from another script now runs only the exported functions, nothing else.

### Verified against every specimen this session has earned, before trusting the wiring at all

AIW (all 12 chapters, 100% recoverable, all four golden scores byte-identical — 61/273, 54/274, 21/257, 50/388), Dorian Gray (ch1-3, 100%), Frankenstein (ch1-3, 100%), Tom Sawyer (ch1, 100%) — all re-read through the NEWLY WIRED code path and byte-identical to their own previously-verified state. Then the actual point of doing this at all: `pg768_The_Adventures_of_Sherlock_Holmes.txt` — the REAL Sherlock Holmes text (corrected by the corpus-integrity pass; its own title-on-same-line convention had already been discovered and persisted to the shared library while building `structure-rec.mjs`, from a scratch copy, never through `eot-jsonl.mjs` itself) — read successfully on the FIRST attempt, `node eot-jsonl.mjs pg768... 1`, with zero hand-editing of any kind. This is the actual payoff named four times and finally collected: a document using a convention this reader was never coded for, working the moment it was tried.

### A second real bug, found extending `recoverability.mjs` to check the new read

`recoverability.mjs` was deliberately kept independent rather than wired to the same shared detector (a verification script sharing its detection code with the thing it verifies loses most of its value as a CROSS-check) — so it needed its OWN, hand-added fourth branch for the title-on-same-line shape, the same as the third branch S110 already added for Tom Sawyer. The first version used `\s+` between the numeral and the title; run against the real Sherlock Holmes text, it silently spanned a blank line and swallowed the OPENING SENTENCE OF THE FIRST PARAGRAPH as if it were the title of a separate, bare "I." heading elsewhere in the same chapter — producing a degenerate `0/0 words (NaN%)` window rather than an error, which would have been easy to mistake for a working result at a glance. Fixed to `[ \t]+` (spaces and tabs only, matching `structure-rec.mjs`'s own already-correct pattern for the identical shape) — re-verified clean: Sherlock Holmes ch1, 8521/8521 words, 679 tiles, 100% recoverable, with zero regression on all four previously-verified specimens.
## S112 — The sentence witness's paraphrase handling, measured by shape at two scales: precision holds 400x, recall does not (2026-09-09)
...
## S113 — legacy-eoreader6.1 partially retired: 86 files migrated off the submodule, the submodule itself NOT removed (2026-09-10)
