...
1. **The address book is a projection of the reader's log, and identity is the reader's.** `EOReferent@1`, `EOMention@1`, the feeders, the reader's own per-sentence surface matcher over its own referents' surfaces; a run resolves by its LONGEST registered surfaces (maximal munch — «Rodya Pyotr Petrovitch» is «Rodya» + «Pyotr Petrovitch», never also «Petrovitch»); fragments fold into beings by every recorded `EOReferentMerge@1` transitively and by the reader's coreference organ placing a partial form inside exactly ONE fuller being (S17 witnessed downward) — a form inside two stays its own, counted. On Crime and Punishment: 113 addresses → 100 beings (8 by record, 5 by containment, 5 ambiguous kept); the presence index (the-fold P38) counted 333.
...
**Corrected 2026-09-08** (a compliance review found it): `holograph-compression.mjs` called `activate`/`lensBlock` with `transcript: []` on every question — the condition the live turn never runs under, since a follow-up question naming no referent of its own is meant to bind to the last answer's. Fixed to thread the run's own transcript row by row; 13 of 25 activating questions became 20 of 25, and the Lens's ceiling share moved from 9 of 13 to 13 of 20. `results/holograph-compression-RESULTS.md` keeps both, dated. The general finding is the same and stronger for it: the ladder compresses, and where it cannot — which on this material is level 1, on every single row — it says so.
