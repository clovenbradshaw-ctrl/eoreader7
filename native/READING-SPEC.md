...
## S128 — a whitepaper's own quotes, archive-anchored: three new organs, no model call, and a live 403 that proved the wall (2026-09-20)

**Generality:** universal — `whitepaper.js`/`archive-anchor.js`/`whitepaper-html.js` read only the ledger's own shape (`end1/label/end2/witnesses/spans`, kernel/notes.js) and archive-anchor.js's own `{quote, sourceUrl}` claim shape; nothing in any of the three names a subject. Demonstrated on two unrelated specimens the same session (a paper about eoreader7 itself; a short stress-test paper about the exoplanet Kepler-452b, with zero domain-specific code) — see `whitepapers/` in this repo for both, plus their shared provenance log.

**The ask, and where the hard part already lived.** The-fold's `explore-server.mjs::archivePage`/`verifySnapshot` (CLAUDE.md's "web organ" section, 2026-08-17) already does the hard, checked half of "a link handed to the reader is verified, never taken on the far side's own word": it saves a Wayback snapshot and then FETCHES and READS it back before ever reporting success. This pass builds the missing piece one level in — composing a DOCUMENT whose own quotes are checked against that mechanism, rather than checking a link a model already wrote.

**Three files, each additive, none touching an existing organ.** `organs/archive-anchor.js` — `anchorClaim`/`anchorClaims`, injected `archive(url)`/`readSnapshot(url)` (this organ owns no network, ranke.js's own stated posture, reused), three and only three standings (`anchored`/`unanchored`/`not_a_url`), a claim's own words checked byte-for-byte against the snapshot text actually fetched (`quoteFoundAt`, exact-substring first, whitespace-normalized fallback, offsets mapped back to the real bytes). `organs/whitepaper.js` — `composeWhitepaper(notesById, {title, sections}, deps)`, refusing outright without a caller-DECLARED section order (compose.js's own wall, the-fold P87, reused rather than re-derived: "an order the caller declares... a guessed order is an argument nobody made"), reading a note's quotable text off its own byte-addressed SPAN (`fold()` in kernel/notes.js drops `because`; the span carries the same material). `organs/whitepaper-html.js` — one static, inert render (no `<script>` anywhere, checked by test), an anchored quote a real `<a>` into the verified snapshot, an unanchored one a visibly distinct `<span>` carrying its own typed reason in its title.

**A real bug, caught by the tests, not by review.** Every one of `anchorClaim`'s returns originally spread `...claim` LAST, so a caller's own claim object could silently clobber the organ's own verdict field — and `whitepaper.js`'s `claimOf` legitimately carries a `standing` field of its own (the note's ledger corroboration standing, kernel/notes.js's `standingOf`), which collided with `ANCHOR_STANDINGS` under that exact name. Found by a real end-to-end test failing with the wrong verdict, not by inspection. Fixed by spreading `...claim` FIRST in every return, so the organ's own decision always wins regardless of what a caller's claim object carries; the whitepaper.js field was also renamed (`ledgerStanding`) so the collision cannot recur even if a future caller forgets the rule.

**A second real bug, the same shape one level up.** `kernel/notes.js::sourceOfWitness`'s own `[kind:]<ref>` grammar reads the text before a witness string's first colon as a kind prefix when it carries no `#`/`~` — which silently strips the scheme off a bare `https://...` witness (`https` read as a "kind"). No existing caller was affected (ranke.js's own convention stores a bare HOST as the witness, never a full URL), but `whitepaper.js`'s own expected input — a note composed straight from web material — legitimately can carry a full URL, so `sourceUrlOf` detects a `https?://`-prefixed witness or span `ref` DIRECTLY, never through `sourceOfWitness`'s kind-stripping, with the ambiguity documented at the point of use rather than fixed in the shared parser (fixing the shared parser would risk the opposite regression for ranke.js's own host-shaped witnesses, out of this pass's scope).

**The adversarial wall, proven, not assumed.** `whitepaper.test.mjs`'s central case constructs a claim whose real, fetched source page does NOT contain the quoted words and confirms both `composeWhitepaper`'s own coverage tally AND the rendered HTML contain zero anchored links for it — the fabricated-quote case this whole feature exists to catch. `archive-anchor.test.mjs` covers five more adversarial conditions (no quote, no source URL, the archive crossing reporting failure, the archive crossing throwing, a "saved" verdict followed by an unreadable snapshot) — 11/11, `whitepaper.test.mjs` 5/5.

**What actually happened running this live, disclosed rather than smoothed over.** `native/eval/the-fold/whitepaper-driver.mjs` composed both real papers and attempted the real archive.org Save Page Now crossing over the network, live, for both — an eoreader7-referencing claim citing arxiv.org's Transformer paper, and the unrelated Kepler-452b claim. Both attempts returned a real, live `HTTP 403` from `web.archive.org/save/...` inside this session's sandboxed network environment. Neither claim was anchored; the whole append-only event log — every attempt, every response — is in `whitepapers/whitepaper-provenance.json`, committed beside the two papers. **This means the success path (a real snapshot verified and quoted from) is proven only against this session's scripted test doubles, not against one live, completed archive.org round trip** — named exactly here, in both whitepapers' own text, and in the provenance log's own `verifications` entries, rather than implied complete. The failure path was proven live, unscripted, against the real service, and it behaved exactly as designed: no fabricated anchor, the real HTTP status disclosed inline in the rendered HTML.

**Verified:** `organs/whitepaper.test.mjs` (5/5), `organs/archive-anchor.test.mjs` (11/11) — all newly added, no existing test file touched, no existing organ imported into by anything new that could regress it. `whitepapers/whitepaper-1-eoreader7.{md,html}`, `whitepapers/whitepaper-2-stress-test.{md,html}`, `whitepapers/whitepaper-provenance.json` are committed as the deliverable and the reproducible record of this pass.
