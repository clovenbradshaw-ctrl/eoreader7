...
test("THE VERDICT: one core file, three domains, zero core edits between them", () => {
  ...
  const src = readFileSync(new URL("./binding-transfer.test.mjs", import.meta.url), "utf8");
  assert.equal((src.match(/import \{ bind, BINDING_CORE_REFUSALS as REFUSALS \} from "\.\/index\.js"/g) ?? []).length, 1, "one import of one core — through the seam");
