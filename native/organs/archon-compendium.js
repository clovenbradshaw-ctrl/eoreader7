...
export const ARCHONS = Object.freeze([
  ...
  },
  {
    ...
    name: "The Claim-Kinds Seam",
    organ: "organs/output-claims.js",
    role: "the four claim-kinds a story needs that plain SVO edges do not carry — causal, temporal, utterance, conditional — each heard from the material's OWN connectives, under the declared language's trigger set",
    pdStatus: "nomination",
    work: "A seam, not a single figure: four archons already in this compendium adjudicate the four kinds (Nagarjuna the consequence, Partee the when, Terry Gross the said, Liu Hui the if-then), and this organ is the one door that hears them all from a sentence's own connectives, language-blind in the core and specialized per language exactly as pronouns.js's PRONOUN_PRIORS holds one entry per language (S39): a language with no registered trigger set is a typed gap, never a silent English match. The words are always the material's; the kind is the organ's. Measured omnilingually against the WLC Tanakh (heb, niqqud folded), Homer's Iliad (grc, polytonic folded), the Latin corpus (la), and English — a false positive is a sentence whose connective the material itself used, never an invented relation.",
    source: "the compendium's own four adjudicating archons; the per-language trigger sets measured from each corpus's real bytes",
    credit: "The claim-kinds seam (output-claims) — causal by Nagarjuna, temporal by Partee, utterance by Terry Gross, conditional by Liu Hui; heard from the material's own connectives, per the material's own language.",
    topics: ["claim-kind", "causal", "temporal", "utterance", "conditional", "connective", "omnilingual", "s39"],
  },
  {
    handle: "output-order",
    name: "The Cut",
    organ: "organs/output-order.js",
    role: "the order an output's claims take — placed by the material's OWN temporal and causal anchors, never by document position passed off as narrative",
    pdStatus: "nomination",
    work: "Murch owns the rhythm of the order (a film is cut where the audience blinks); Vonnegut owns the spine (the 27-operator arc, taxonomically complete). The order is the cut; the spine is the arc. The material's temporal kinds (output-claims.js) carry first/then/afterward; this organ places each claim in the temporal window that contains its span, and REFUSES when the material states no order — document order is not narrative order, and this organ will not pass one off as the other.",
