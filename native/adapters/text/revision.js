 ...
 */
export async function reviseTextFold({ observations = [], fold = {}, graph = null, index = null, canonicalizationFloor = undefined } = {}) {
  ...
  // the SAME support/attack grammar apposition and the discourse index feed
  // — one identity-revision door, not a second mechanism. Recall-based
  // anchoring and discourse-based binding resolve identity differently and
  // are strongest on different material.
  //
  // BEFORE the guard below, deliberately: an anchor is first-class evidence
  // and must be able to OPEN identity revision on its own. Placed inside the
  // guard it could only ever add to a block some other evidence had already
  // opened — caught by anchoring.test.js's own end-to-end case.
