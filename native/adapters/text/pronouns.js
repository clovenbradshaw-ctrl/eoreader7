...
import { THIRD_PERSON_SINGULAR, THIRD_PERSON_SINGULAR_META } from "./priors.js";
...
/**
 * Third-person-singular pronoun mentions in `text`, read under `language`'s
 * own closed class (declared, giver named). A language with no registered
 * prior returns no hits — exactly the outcome the English regex already
 * produced by accident against a script it cannot match; the difference is
 * that the decision is now declared rather than a side effect of which
 * characters happened not to collide.
 *
 * @param {string} text
 * @param {string} [language="en"]
 */
export const findThirdPersonSingular = (text, language = "en") => {
  const prior = pronounPriorFor(language);
  if (!prior) return [];
  const hits = [];
  for (const m of text.matchAll(prior.re)) {
    const token = m[0].toLowerCase();
    hits.push({ token, gender: prior.table[token], index: m.index });
  }
  return hits;
};
...
export const resolvePronouns = (
  ...
  for (const sentence of sentences ?? []) {
    ...
    const pronounHits = findThirdPersonSingular(sentence.text, language);
...
export const resolvePronounsByActivation = (
  sentences,
  referentSurfaces,
  { window, minActivation, minMargin, language = "en", nonPersonal, createActivation } = {},
) => {
