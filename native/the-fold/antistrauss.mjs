...
//      upstream fetch; review() runs when the stream ends.
...
const SAFETY_LAWS = [
  ...
  },
  {
    ...
    basis: "step-by-step instructions to build an operational weapon (explosive, improvised firearm) for the purpose of harming people",
    patterns: [
...
const LAW_EDGES = CLASS_IDS.map((id, i) =>
  hyperedge({
    ...
    witness: `antistrauss-physics:${GROUND_DIGEST}`,
...
chemistry = giveHyperlexiconAffordance(chemistry, {
  left: "mentions", right: "is", giver: GIVER, binding: GROUND_DIGEST,
...
for (const row of closureAffordances({ base: "contravenes", yields: "contravenes", giver: GIVER })) {
  chemistry = giveHyperlexiconAffordance(chemistry, { ...row, binding: GROUND_DIGEST });
}
...
// spans are the evidence. Plain text, no model involved.
...
//   { allow, mode, reason, verdict } where verdict carries the full
// physics report (steps, derived contraventions, provenance, ground).
export function gate({ model, messages = [], route = "chat", sessionId = null, workspace = null } = {}) {
  ...
  const verdictBase = {
    ...
    mode: MODE,
    ground: { file: PHYSICS_FILE.split("/").pop(), digest: GROUND_DIGEST, marker: GROUND_MARKER },
    physics: { substrate: "createReactionSubstrate (native/kernel/reaction.js)", ...physics },
  };

  if (MODE === "off") {
    return { allow: true, mode: MODE, reason: "gate open by ER7_ANTISTRAUSS=off", verdict: { ...verdictBase, promptDigest, hits: [] } };
  }

  const hits = scanPrompt(text);
  if (!hits.length) {
    return { allow: true, mode: MODE, reason: "no prohibited class declared", verdict: { ...verdictBase, promptDigest, hits: [], settled: null } };
  }

  // ── tier 2: the physics settles the verdict. The sensor's hits are
  // admitted as raw `mentions` edges (provenance = the detection bytes);
  // the cue lights the classes; the substrate derives — or refuses to
  // derive — the `contravenes` edge. The verdict is the settle's own
  // derived facts, never a number invented here.
  const mentionEdges = hits.map((h, i) =>
    hyperedge({
      id: `antistrauss:det:${at}:${i}:${h.class}`,
      relation: "mentions",
      participants: [
  ...
  const blocked = contravening.length > 0;

  const verdict = {
    ...verdictBase,
    promptDigest,
    hits: hits.map((h) => ({ class: h.class, label: idByLaw.get(h.class)?.label ?? h.class, basis: idByLaw.get(h.class)?.basis ?? null })),
    settled: {
  ...
  if (MODE === "log") {
    appendLog({ act: "gate", ...verdict, refused: false, wouldRefuse: blocked || ungrounded });
    return { allow: true, mode: MODE, reason: blocked || ungrounded ? "would refuse in block mode (ER7_ANTISTRAUSS=log)" : "no contravention", verdict };
  }
  appendLog({ act: "gate", ...verdict, refused: false });
  return { allow: true, mode: MODE, reason: blocked ? "contravention recorded but mode=block did not fire" : "no contravention", verdict };
}
...
export function review({ model, messages = [], output = "", route = "chat", sessionId = null, workspace = null, verdict = null, ok = true } = {}) {
  const text = (messages ?? []).map((m) => String(m?.content ?? "")).join("\n");
  appendLog({
    act: "review",
...
export function status() {
  return {
    ...
    label: "AntiStrauss",
    mode: MODE,
    wired: ["proxy-runner.mjs::streamOllamaChat", "proxy-runner.mjs::discoverFraming (routed through the gate)"],
    physics: {
      substrate: "createReactionSubstrate (native/kernel/reaction.js)",
      ...physics,
      ground: { file: "antistrauss-physics.txt", digest: GROUND_DIGEST, marker: GROUND_MARKER },
...
export const antistrauss = { gate, review, status, MODE };
