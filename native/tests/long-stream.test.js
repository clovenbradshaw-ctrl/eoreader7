...
import { answerAtoms, buildFactBank, makeRng, recallProbe, scoreRecall, memoryProbe, scoreMemory, injectionProbe, scoreInjection, reasoningProbe, scoreReasoning, organicQuestion, scheduleFor, memoryDistanceFor, PROBE_KINDS, FOLD_UNAVAILABLE } from "../eval/the-fold/lib/long-stream.mjs";

// A DRIVER REFUSES WHAT ITS CHECKOUT LACKS (READING-SPEC S65 / the-fold
// P95): every test below reaches `atomsOf`/`checkSentence`/`numberSet`,
// which need a sibling `the-fold` checkout this repo's own CI never
// checks out. `FOLD_UNAVAILABLE` (long-stream.mjs's own typed check) is
// read once, here, so a missing sibling is a real, reported skip on
// every test rather than each one failing with the same thrown error.
const SKIP = FOLD_UNAVAILABLE ?? undefined;
...
test("the bank holds sentences with a number and a name, addressed; the rng is reproducible", { skip: SKIP }, () => {
...
test("recall: the cloze blanks exactly the atom; hit / wrong / miss are read mechanically; an unrelated answer is not a hit (control)", { skip: SKIP }, () => {
...
test("memory: the earlier answer's atoms are the ground; share and contradiction are mechanical", { skip: SKIP }, () => {
...
test("injection: the false premise moves one atom; held / capitulated / both / evaded", { skip: SKIP }, () => {
  const bank = buildFactBank(chunks, { perSource: 5, rng: makeRng(1) });
...
test("reasoning: two sources, an exact difference; right / partial / wrong; same-source pairs are refused", { skip: SKIP }, () => {
  const bank = buildFactBank(chunks, { perSource: 5, rng: makeRng(1) });
...
test("the schedule rotates the four kinds every fifth turn; organic turns ask off the bank or follow up; memory distance never reaches before turn 1", { skip: SKIP }, () => {
...
test("the bank refuses markup and critical apparatus — a manuscript variant is not a proposition (2026-09-06)", { skip: SKIP }, () => {
...
test("the bank refuses bibliography entries; an answer's atoms are read per sentence, and capitalised function words are not names (control: the reference-list fragment yields no fact)", { skip: SKIP }, () => {
