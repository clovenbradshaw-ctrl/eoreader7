...
export function extractGfpRelations(text, { posPrior = null, functionWords = null, minRec = 2, figures = null } = {}) {
  ...
  for (let i = 0; i < mentions.length - 1; i += 1) {
    ...
    // A ZERO-LENGTH CONNECTOR IS THE TIGHTEST ADJACENCY, NOT AN ERROR
    // (2026-09-19, falsified live in the 25-language existence battery):
    // "ferir pessoas" — a harm figure and a person figure, ADJACENT — read as
    // `{ferir} —()→ {pessoas}`, and dropping it starved every short weapon ask
    // into an empty reading. The GFP definition names adjacency as the
    // arrangement ("two figures whose mentions ADJOIN in reading order, the
    // text between them the connector"); a zero-length connector is still
    // text-between-nothing, the closest possible bond. Kept only when the two
    // figures are DISTINCT tokens (a self-collision like "step by step"
    // emitting `{step}—()→{step}` is a duplicate, not an arrangement) and the
    // label stays within the declared bound.
    if (label === f2.tok) continue; // no connector at all — a self-collision, never an arrangement
    const labelHead = String(label).split(/[^-\p{L}\p{N}]+/u).filter(Boolean)[0];
    if (!labelHead) {
      // EMPTY-CONNECTOR ADJACENCY: keep it only when the ends are different
      // tokens and genuinely adjacent (nothing but whitespace between them).
      if (f2.start - f1.end <= 1 && f1.tok.toLowerCase() !== f2.tok.toLowerCase()) {
        // typed as a grain gap — the connector could not settle (there was no
        // connector); kept, never guessed. The kernel's cell for a zero-label
        // link is the reader's own disclosure.
        out.push({
          end1: f1.tok,
          label: "",
          end2: f2.tok,
          cell: "grain_gap",
          grain: { grain_gap: "zero-length connector — adjacent figures, connector could not settle", settledAs: null },
          polarity: "+",
          offset: f1.end,
        });
      }
    ...
    if (label.length > MAX_LABEL) continue;
