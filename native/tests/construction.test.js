...
test("the form-level prior holds `had` in superposition — the fact the gate destroys", { skip: SKIP }, () => {
...
test("`had` before a participle collapses AUX — the passive the type-level test admits", { skip: SKIP }, () => {
...
test("`had` before a determiner collapses VERB — the SAME form, the other reading", { skip: SKIP }, () => {
...
test("a cell that does not clear minShare stays LIVE — the superposition did not collapse", { skip: SKIP }, () => {
...
test("minShare is declared, never defaulted", { skip: SKIP }, () => {
...
test("an unattested form is a typed gap, never a guess", { skip: SKIP }, () => {
...
test("backoff to the form level is disclosed as a weaker basis, never mixed with the construction", { skip: SKIP }, () => {
...
test("the end of a sentence is a frame, not a missing value", { skip: SKIP }, () => {
  const out = collapseForm("had", null, opts(2 / 3));
  assert.equal(out.frame, constructionPrior.declared.sentenceEndFrame);
});

test("the received prior names its giver and its licence", { skip: SKIP }, () => {
