...
const prior = defaultLatinCasePrior();

test("OMNIMODAL CLAIM, CHECKED DIRECTLY: a real held-out VOS sentence, verb-object-subject order", () => {
...
test("a second real free-order sentence: subject second, object last, verb third", () => {
...
test("the shape is {end1, label, end2} natively -- never subject/verb/object", () => {
  const r = extractCaseMarkedRelation("Maxima pars unda rapitur.", { casePrior: prior });
...
test("a multi-verb sentence is a typed gap, never a guess at which verb is the clause's own", () => {
...
test("no finite verb at all is a typed gap, never an empty guess", () => {
  const r = extractCaseMarkedRelation("in via.", { casePrior: prior });
  assert.equal(r.gap?.reason, "no_verb_found");
});

test("a weak (single-character) verb ending withdraws when the same word also reads as a confident nominal case", () => {
...
test("an oblique (dative/ablative/genitive) participant is typed distinctly, never folded into end2", () => {
...
test("an unattested word ending returns a gap, never a guess dressed as a case", () => {
...
test("verbHint isolates role-assignment from verb-finding, for measurement only", () => {
  const r = extractCaseMarkedRelation("Maxima pars unda rapitur.", { casePrior: prior, verbHint: "rapitur" });
  assert.equal(r.label.word, "rapitur");
  assert.equal(r.end1?.word, "pars");
});

test("the prior itself: real provenance, giver named, license disclosed plainly", () => {
