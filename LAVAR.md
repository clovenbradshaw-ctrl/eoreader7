...
**Files.** `native/kernel/hyperlexicon.js` (`compositionAffordance` gains the grain-keyed structural fallback), `native/kernel/reaction.js` (passes the chains' `eo.grain`), `native/eval/lavar/prime-with-field.mjs` (declares the structural affordance). Suite: 778/772/6, same six pre-existing failures, zero new.
