...
Searched for more of the same class of gap Houdini already fixed once (a leak slipping past a check via a different code path), per direct instruction to find more places for him to debunk. Found one, real and concrete, by reading (not assuming): proxy-runner.mjs's own live essay-writer (WOLFE) has a SECOND, independent admission mechanism from admission.js's admit() -- a "two roads" (matter/motion) loop that correctly calls the real isMetaSentence unconditionally, sitting right beside a DIFFERENT branch (the "open sentence" fallback path) whose PRIMARY filter used a hand-inlined regex holding only referent-verify.js's oldest pattern (task-talk phrases). That inline copy had silently fallen behind its own source twice: it lacked ASSISTANT_VOICE_RE (added 2026-09-24) and TECHNIQUE_COMMENTARY_RE (added earlier this session, after the real "Embedded Information" leak). A sentence passing this stale filter shipped directly as `buf` -- the real, complete isMetaSentence in this same function only ran later, on entirely different candidates, in a fallback branch reached only when the stale filter admitted nothing at all.

Confirmed live, mechanically, before and after: the real isMetaSentence correctly catches the leak, chat-voice phrasing, and the original task-talk pattern; the stale inline regex would only have caught the last of those three. Fixed by replacing the inline regex with a direct call to the same isMetaSentence function the sibling admission loop 20 lines above already uses correctly -- removing the drift risk itself rather than patching the regex a third time. Grepped the rest of the codebase for any other hand-copied instance of the same pattern: none found: this was the only other copy.

46/46 existing tests (referent-verify-falsify, archon-rules-houdini, admission-falsify) pass unchanged. No archon dispute: proxy-runner.mjs's this specific branch is named by no live archon (registry.mjs: 0 live archons, checked before editing).

Still true from last cycle: the live daemon has not been restarted, so neither this fix nor the earlier Houdini wiring has been exercised by an actual live turn yet.

> Direct instruction to find more of the same class of gap; found one by reading, not guessing, a second independent admission mechanism whose primary filter had drifted behind its own real source twice, fixed by calling the real function instead of patching the copy again, verified mechanically before and after.
